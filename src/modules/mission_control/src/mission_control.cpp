...
MissionControl::MissionControl()
  ...
  timer_ = this->create_wall_timer(
      milliseconds(300), std::bind(&MissionControl::timerCallback, this));
...
void MissionControl::postConstructionSetup() {
  ...
  while (rclcpp::ok() && !set_mode_client_->wait_for_service(seconds(1))) {
    RCLCPP_INFO(this->get_logger(), "Waiting for mode setting service...");
  }
  while (rclcpp::ok() && !arm_client_->wait_for_service(seconds(1))) {
...
void MissionControl::stateCallback(
    const mavros_msgs::msg::State::SharedPtr msg) {
  current_state_ = *msg;
}

void MissionControl::positionCallback(
    const nav_msgs::msg::Odometry::SharedPtr msg) {
  current_position_ = msg->pose.pose;
}

void MissionControl::velocitySetpointCallback(
...
bool MissionControl::checkExternalCmdTimeout() {
  if (external_velocity_command_ &&
      (this->get_clock()->now() - last_external_cmd_time_).seconds() >
          external_cmd_timeout_) {
    external_velocity_command_ = nullptr;
    return true;
  }
  return false;
...
void MissionControl::setMode(const std::string &mode) {
  ...
  if (rclcpp::spin_until_future_complete(this->shared_from_this(), future) ==
    ...
    if (response->mode_sent) {
      RCLCPP_INFO(this->get_logger(), "Mode changed to %s successfully.",
                  mode.c_str());
    } else {
...
void MissionControl::arm(bool value) {
  ...
  const milliseconds retry_delay(1000);

  int attempt = 0;
  bool success = false;

  while (attempt < max_retries && !success) {
    RCLCPP_INFO(this->get_logger(), "Arming attempt %d of %d...", attempt + 1,
                max_retries);
  ...
  if (!success) {
    RCLCPP_ERROR(this->get_logger(), "Failed to arm after %d attempts.",
                 max_retries);
  }
}
...
void MissionControl::timerCallback() {
  ...
  if (!armed_) {
    ...
    RCLCPP_INFO_ONCE(this->get_logger(),
                     "Vehicle not armed. Waiting for arming...");
  ...
  if (mission_state_ == "INIT") {
    if (current_position_.position.z < cruise_altitude_) {
      ...
      RCLCPP_INFO(this->get_logger(),
                  "Reached cruise altitude, transitioning to CRUISE.");
    }
  } else if (mission_state_ == "CRUISE") {
    if (!checkExternalCmdTimeout() && external_velocity_command_) {
      vel_cmd.twist =
          external_velocity_command_->twist;  // Apply external velocity command
    } else {
      vel_cmd.twist.linear.x = 0.0;
      vel_cmd.twist.linear.y = 0.0;
